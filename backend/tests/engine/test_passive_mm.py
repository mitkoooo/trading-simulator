...
def test_get_mid_history(sample_exchange: Exchange):
    ...
    mq = MarketQuote(SYMBOL, bid_price=None, bid_size=1, ask_price=None, ask_size=1,last_price=100, timestamp=datetime.now())
...
def test_on_book_update_mid_on_market_orders_quote_no_last(sample_exchange: Exchange):
    SYMBOL = "AAPL"

    passive_mm = PassiveMM(sample_exchange, SYMBOL, "MM_TEST")

    mq = MarketQuote(SYMBOL, bid_price=None, bid_size=1, ask_price=None, ask_size=1,last_price=None, timestamp=datetime.now())
...
def test_on_book_update_mid_on_quote_empty(sample_exchange: Exchange):
    ...
    mq = MarketQuote(SYMBOL, bid_price=None, bid_size=0, ask_price=None, ask_size=0,last_price=None, timestamp=datetime.now())
...
def test_on_book_update_mid_on_quote(sample_exchange: Exchange):
    ...
    mq = MarketQuote(SYMBOL, bid_price=101, bid_size=1, ask_price=99, ask_size=1, last_price=None, timestamp=datetime.now())
...
def test_compute_quote(sample_exchange: Exchange):
    ...
    bid_price, ask_price = passive_mm.quote_engine.compute(mid, vol, depth_imb, inventory)
